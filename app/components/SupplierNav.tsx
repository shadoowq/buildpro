...
const NAV_LINKS = [
  { icon: '🏠', labelAr: 'لوحة التحكم',    labelEn: 'Dashboard',         href: '/supplier-dashboard' },
  { icon: '📋', labelAr: 'الطلبات المتاحة', labelEn: 'Available Requests', href: '/supplier-requests'  },
  { icon: '📄', labelAr: 'عروضي',          labelEn: 'My Quotes',          href: '/my-quotes'          },
  { icon: '📊', labelAr: 'التقارير',       labelEn: 'Reports',            href: '/supplier-reports'   },
  { icon: '🗑', labelAr: 'سلة المهملات',   labelEn: 'Trash',              href: '/supplier-trash'     },
];
    ...
    <div className={`${positionCls} w-80 bg-white border border-[var(--line)] rounded-2xl shadow-xl z-50 overflow-hidden`} dir={dir}>
      <div className="flex items-center justify-between px-4 py-3 border-b border-[var(--line-soft)]">
        ...
        <Link href="/notifications" onClick={() => setBellOpen(false)}
          className="text-[11px] text-[var(--sec)] font-semibold hover:underline">
  ...
  return (
    <>
      ...
      <aside className="hidden md:flex fixed top-0 bottom-0 start-0 w-[190px] z-40 flex-col bg-[var(--chrome)] px-3 py-5" dir={dir}>
        <Link href="/supplier-dashboard" className="text-lg font-bold text-[var(--brand)] px-2 pb-5 shrink-0">
      ...
      <nav className="md:hidden bg-white border-b border-[var(--line)] px-4 flex items-center justify-between h-14 sticky top-0 z-30" dir={dir}>
        ...
        {/* logo */}
        <Link href="/supplier-dashboard" className="text-[17px] font-bold text-[var(--brand-strong)] shrink-0">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <div className="hidden sm:flex items-center gap-1 bg-stone-100 rounded-xl p-1">
            {(['ar', 'en'] as Lang[]).map(l => (
              <button key={l} onClick={() => setLang(l)}
                className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs font-bold transition-all ${lang === l ? 'bg-white text-[var(--brand-strong)] shadow-sm' : 'text-stone-500 hover:text-stone-600'}`}>
              ...
              <div className="flex gap-1 px-4 py-2">
                {(['ar', 'en'] as Lang[]).map(l => (
                  <button key={l} onClick={() => { setLang(l); setMenuOpen(false); }}
                    className={`flex-1 flex items-center justify-center gap-1.5 py-2 rounded-lg text-xs font-bold border transition-all ${lang === l ? 'bg-[var(--tint)] text-[var(--brand-strong)] border-[var(--brand-strong)]/20' : 'border-[var(--line)] text-stone-500'}`}>
                    <img src={l === 'ar' ? 'https://flagcdn.com/w20/sa.png' : 'https://flagcdn.com/w20/us.png'} width="18" height="13" alt={l} className="rounded-sm" />
                    {l.toUpperCase()}
                  </button>
                ))}
